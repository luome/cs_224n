...
def weighting(w_i, w_j):
    ...
    # the following two parameters are recommended by the artice
    x_max = 100
    alpha = 0.75
...
def prepare_word(word, word2index):
    return torch.tensor([word2index[word]], dtype=torch.long) if word2index.get(word) is not None else torch.tensor(
        [word2index["<UNK>"]])
